...
class GoogleTranslate:
    def __init__(self, target, origin_lang, expect_lang):
        self._target = target
        self._origin_lang = origin_lang
        self._expect_lang = expect_lang
    ...
    def url_serialize(self, key):
        ...
        return ''.join(["https://translate.google.ru/translate_a/single?"
                        "client=t&",
                        "sl={}&".format(origin_lang),
                        "tl={}&".format(expect_lang),
                        "hl=ru&",
                        "dt=bd&",
                        "dt=ex&",
                        "dt=ld&",
...
class UrbanDict:
    ...
    def get_top_definition(self):
        return self.get_all_definitions()[0]

    def get_definitions(self, count=None):
        return [d.get("definition") for d in self.response["list"]][:count] or ["no results"]

    def show(self, count=None):
        for definition in self.get_definitions(count):
            print("\n", definition)
